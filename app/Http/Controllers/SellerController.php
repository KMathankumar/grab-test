...
// }
    ...
    public function showTenMinProducts(Request $request)
    {
        // Only categories that have at least one IN-STOCK 10-min product
        $categories = Category::whereHas('tenMinProducts', function ($query) {
            ...
            ->get()
            ->map(function ($cat) {
                // Remove empty categories (edge case)
                if ($cat->tenMinProducts->isEmpty()) {
                    return null;
                }

                // Compute used subcategories from in-stock products only
                $subcategories = $cat->tenMinProducts
                    ->pluck('subcategory')
                    ->filter()
                    ->unique('id')
    ...
    // Determine active category safely
        ...
        // Prepare JS data (only in-stock products)
        $jsCategories = $categories->map(function ($cat) {
            return [
                'id' => $cat->id,
                'name' => $cat->name,
    ...
    public function show($id)
    {
        // Safe: Only in-stock products are ever linked, but double-check
        $product = TenMinDeliveryProduct::with(['category', 'subcategory', 'seller'])
            ->where('stock', '>', 0)
            ->findOrFail($id);
