...
<div class="page-wrapper">
            ...
            <div class="categories-scroll">
                @foreach($foodCategories as $cat)
                    <a href="?category={{ $cat['id'] }}" class="category-card text-decoration-none">
                        <img src="{{ asset('category_icons/' . ($cat['icon'] ?? 'placeholder.png')) }}" 
                             onerror="this.src='https://via.placeholder.com/85?text={{ urlencode($cat['name'][0]) }}'" />
    ...
    <div class="d-flex justify-content-between align-items-center">
        <div class="controls-row">
            @if(request()->hasAny(['search', 'category', 'veg', 'sort']))
...
<h5 class="mt-3">
    ...
    @elseif(request('category'))
        @php
            ...
            $catName = $foodCategories->firstWhere('id', $catId)['name'] ?? ucfirst(str_replace('_', ' ', $catId));
        @endphp
        {{ $catName }}
    @else
        All Food Items
        ...
        @forelse($foods as $food)
            <div class="col-sm-responsive p-2">
                <a href="{{ route('customer.food.details', $food->id) }}" class="text-decoration-none">
                    <div class="food-card">
                        @php
                            $imageUrl = 'https://via.placeholder.com/480x300?text=No+Image';
                            if (!empty($food->images) && is_array($food->images) && isset($food->images[0])) {
                                $imageUrl = $food->images[0];
                            }
                        @endphp
                        <img src="{{ $imageUrl }}" onerror="this.src='{{ $imageUrl }}'">
                        ...
                        <div class="mt-2">
                            <i class="fa-solid fa-location-dot"></i>
                            {{ $food->hotelOwner?->restaurant_name ?? 'Unknown Restaurant' }}
                        </div>
                    </div>
                </a>
            </div>
        @empty
            <div class="col-12 text-center py-5">No food items available right now.</div>
        @endforelse
    </div>
</div>
