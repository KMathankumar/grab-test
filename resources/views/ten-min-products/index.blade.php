...
<head>
    ...
    <style>
        ...
        .category.active .badge {
            background: linear-gradient(135deg,#e6f7e6,#eafbed);
            box-shadow: 0 18px 40px rgba(39,122,39,0.12);
        }
        ...
        .sub-item:hover {
            ...
            background: linear-gradient(90deg,#fbfff6,#fff);
        }
        .sub-item.active {
            background: linear-gradient(90deg,#ecffd6,#fff);
            box-shadow: 0 10px 30px rgba(47,122,47,0.06);
        }
        .sub-item .icon {
            width: 44px;
            height: 44px;
        ...
        .thumb {
            ...
            height: 150px;
            border-radius: 10px;
            background: linear-gradient(180deg,#eef4f7,#f7fbfb);
            display: flex;
            align-items: center;
            justify-content: center;
            overflow: hidden;
        ...
        .ribbon {
            ...
            top: 12px;
            right: 12px;
            background: linear-gradient(180deg,#ff6b6b,#e33434);
            color: #fff;
            padding: 6px 8px;
            border-radius: 8px;
            font-weight: 700;
            font-size: 12px;
            box-shadow: 0 8px 22px rgba(227,52,52,0.16);
        }
        .card h4 {
            margin: 12px 0 6px 0;
            font-size: 15px;
        ...
        @media (max-width: 900px) {
            ...
            .header-row { align-items: center; padding-left: 12px; padding-right: 12px; }
            .categories {
                display: grid;
    ...
    {{-- 🔑 Inject correct product detail route for JS --}}
    <script>
        window.routes = {
            productDetails: (id) => "{{ route('product.details', ['id' => '__ID__']) }}".replace('__ID__', id)
        };
    </script>
</head>
<body>
...
<header>
    <div class="container header-row">
        ...
        <div class="header-actions">
            @auth
                <a href="{{ route('tenmin.cart.view') }}" style="color:#0b1a12;text-decoration:none;font-weight:600">Cart</a>
    ...
    <div class="container">
        <div id="categoriesContainer" class="categories">
            @foreach($categories as $cat)
                ...
                @endphp
                <a href="{{ route('ten.min.products', ['category' => $cat->id]) }}" class="category {{ $isActive ? 'active' : '' }}">
                    <div class="badge">
                        <img src="{{ $fallback }}" alt="{{ $cat->name }}" />
                    </div>
    ...
    <div class="layout container">
        ...
        <section>
            <div class="catalog-head">
                <div>
                    <h2 id="catalogTitle">{{ $activeCategory?->name ?? 'Products' }}</h2>
                    <div style="color:#6b7280;font-size:13px">
                        Showing <span id="showCount">{{ $activeCategory?->tenMinProducts->count() ?? 0 }}</span> items
                    </div>
                </div>
                <div>
                    <select id="sortBy">
...
let activeCategory = categoriesData.length > 0
    ? (categoriesData.find(c => c.id == {{ $activeCategory?->id ?? 'null' }}) || categoriesData[0])
    : null;
let activeSub = 'All';
...
function renderProducts() {
    ...
    const products = activeCategory.products.filter(p =>
    ...
    if (products.length === 0) {
        ...
        products.forEach(p => {
            ...
            card.innerHTML = `
                ...
                <a href="${window.routes.productDetails(p.id)}" style="text-decoration:none;color:inherit;display:block;">
                    <div class="thumb">
...
document.getElementById('searchBtn')?.addEventListener('click', () => {
    ...
    const productGrid = document.getElementById('productGrid');
    productGrid.innerHTML = '';
    const matched = activeCategory.products.filter(p =>
