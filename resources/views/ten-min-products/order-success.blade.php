...
<div class="container">
    <div class="card">
        ...
        <p>Thank you for your order. It's being prepared now.</p>

        <!-- ✅ SINGLE ORDER (no foreach) -->
        <div class="order-details">
            <p><strong>Order ID:</strong> #{{ $order->id }}</p>
            <p><strong>Store:</strong> {{ $order->seller?->name ?? 'GrabBasket Store' }}</p>
            <p><strong>Total Amount:</strong> ₹{{ number_format($order->total_amount, 2) }}</p>
            <p><strong>Estimated Delivery:</strong> {{ $order->estimated_delivery_time?->format('d M Y, h:i A') ?? 'In 10 minutes' }}</p>
        </div>
